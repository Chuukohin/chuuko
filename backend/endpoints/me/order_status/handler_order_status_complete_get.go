...
// OrderStatusCompleteHandler
// @ID           me.order_status.complete.get
// @Summary      Get all products that are completed
// @Description  Get all products that are completed
// @Tags         me_order_status
// @Accept       json
// @Produce      json
// @Success      200  {object}  []order_status.orderStatusToReceive
// @Failure      400  {object}  responder.ErrorResponse
// @Router       /me/order_status/complete [get]
func OrderStatusCompleteHandler(c *fiber.Ctx) error {
	...
	if result := database.Gorm.Raw("SELECT od.id order_id, od.order_time, p.name product_name, p.price price, pic.picture_url product_picture "+
		...
		"WHERE od.user_id = ? "+
		"AND odt.received = 'true'", claims.UserId).Scan(&orders); result.Error != nil {
